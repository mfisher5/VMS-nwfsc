---
title: "Pre-Processed Fish Tickets for VMS match"
author: M. Fisher
...
knitr::opts_knit$set(root.dir = here::here(), echo=TRUE) # will set root directory to repository root folder
```
...
```{r rawdata}
rawdat <- read.csv(paste0(rawdir,myfile), stringsAsFactors = FALSE)
rawdat <- rawdat %>%
...
First, subset the raw data to include only the columns that are needed. Remove the columns where the vessel identifier (vessel number) is either "UNKNOWN" or blank ("")
```{r}
rawdat.sub <- rawdat %>% dplyr::select(rawdat, c(FISH_TICKET_ID, PACFIN_PORT_CODE, PACFIN_GROUP_PORT_CODE, VESSEL_NUM, AGENCY_CODE, GEAR_CODE, GEAR_NAME, PACFIN_GROUP_GEAR_CODE, REMOVAL_TYPE_CODE, REMOVAL_TYPE_NAME, LANDING_DATE, LANDING_MONTH, LANDING_YEAR, PACFIN_SPECIES_CODE, LANDED_WEIGHT_LBS, EXVESSEL_REVENUE)) %>%
  filter(VESSEL_NUM != "UNKNOWN" & VESSEL_NUM != "")
```
